<template>
	<view>
		...
		<card headTitle="发票类型" bodyPadding cardStyle="background:#ffffff">
			<ZradioGroup :radioLabel="label1" :selected.sync="label1.selected" />
			<view class="text-light-muted line-h-md">
				电子发票与纸质发票具有相同法律效力，可作为报销、售后、维权凭证，推荐使用电子发票，不怕丢失，更方便、环保。
			</view>
		</card>
		<myline></myline>
		<card headTitle="发票抬头" bodyPadding cardStyle="background:#ffffff">
			<ZradioGroup :radioLabel="label2" :selected.sync="label2.selected" />
		</card>
		<myline></myline>
		...
		<view class="p-2 border-bottom d-flex a-center bg-white">
			...
			<text class="font-md ml-2">188****8888</text>
		</view>
		<view class="p-2 text-light-muted d-flex flex-column">
			<text>发票须知：</text>
			<text>1.发票为实际支付金额，不包含优惠劵等；</text>
